...
However, this approach introduces a critical problem previously noted by the framework's authors: if a concurrent thread modifies our frame by executing a CAS on an internal node rather than the root, data structure consistency can be compromised. Such an operation cannot detect the root-level CAS that replaced the entire frame. Consequently, the concurrent thread becomes disconnected from the main tree root because the nodes it was operating on are permanently removed from the active topology, having been replaced by our local frame copy. To prevent this inconsistency, method descriptors were introduced. To execute an operation, a thread creates a descriptor encapsulating its current data structure operation. Subsequently, a reference to this descriptor is attached to all nodes involved in the operation. If another thread attempts to access a marked node, it must first inspect the descriptor and help the original thread complete its execution. This helping mechanism must be applied recursively if executing the helping operation requires assisting a third operation, and so on. This helping mechanism, akin to the multi-word CAS approach by Ellen et al. \cite{NonBlockBST}, ensures both system-wide progress and strict linearizability \cite{Linearizability} during concurrent modifications.

We formalize this frame-based update in TLA+ to ensure its correctness. The \texttt{TreeCAS} module is modeled around a minimal tree consisting of a root and its child leaf. This structure is sufficient to verify the framework's correctness, as the critical race condition to evaluate involves a larger frame overlapping a smaller one (furthermore, scaling the state space to include more nodes does not break the framework, as confirmed by our larger-scale model checking runs). To model the allocation of new nodes, we utilize an infinite memory pool constrained by a state-space filter limiting the maximum number of active allocated cells to six. The overall algorithm comprises five distinct phases, with the third phase implementing the recursive helping mechanism. This phase is structured as follows:
...
Help(d, n) == LET otherD == nodeDesc[n] IN
    ...
    /\ IF descState[otherD] \in {"ACTIVE", "COMMITTING"}
       THEN CommitStep(otherD)
       ELSE IF descState[otherD] \in {"SUCCESS", "ABORTED"}
       THEN ReleaseDescriptor(otherD)
       ELSE UNCHANGED vars
\end{tla}
\begin{tlatex}
\@x{}%
...
\@xx{}%
 ...
 \.{\IN}}%
\@x{ \.{\land} descState [ d ] \.{=}\@w{ACTIVE}}%
\@y{%
  We can only offer help if 
}%
\@xx{}%
\@x{ \.{\land} n \.{\in} descPending [ d ]}%
\@y{%
...
\@xx{}%
 \@x{ \.{\land} {\IF} descState [ otherD ] \.{\in} \{\@w{ACTIVE}
 ,\,\@w{COMMITTING} \}}%
\@x{ \.{\THEN} CommitStep ( otherD )}%
 \@x{ \.{\ELSE} {\IF} descState [ otherD ] \.{\in} \{\@w{SUCCESS}
 ,\,\@w{ABORTED} \}}%
\@x{ \.{\THEN} ReleaseDescriptor ( otherD )}%
\@x{ \.{\ELSE} {\UNCHANGED} vars}%
\end{tlatex}

As demonstrated here, we do not distinguish between the concept of an execution thread and the descriptor of the operation it performs. This abstraction can be interpreted as the use of coroutines within the physical production code, a pattern that is highly prevalent in practice. An interesting aspect of constructing formal specifications for non-blocking algorithms is that the \texttt{Help(d, n)} routine is only strictly necessary in the actual implementation. While omitting it compromises the global progress property, it is not mathematically required to maintain safety invariants.
...
\@x{ GlobalProgress \.{\defeq}}%
 \@x{\@s{16.4} \E\, d \.{\in} Descs \.{:} descState [ d ] \.{=}\@w{ACTIVE}
 \.{\leadsto}}%
\@x{\@s{20.5} \E\, u \.{\in} Descs \.{:} descState [ u ] \.{=}\@w{SUCCESS}}%
\@x{\@s{24.6} \.{\lor} {\lnot} FreeStructConstraint}%
\end{tlatex}
