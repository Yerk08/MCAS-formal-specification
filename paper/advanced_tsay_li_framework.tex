...
Help(d, n) == LET otherD == nodeDesc[n] IN
  ...
  /\ IF descState[otherD] \in {"ACTIVE", "COMMITTING"}
     THEN (* If the competitor is in progress *)
        CommitStep(otherD) (* then make the next step *)
...
/\ IF nodeStruct[n] /= descExpectedStruct[d][n]
  ...
  THEN ExecuteAcquire(d, n) (* CAS SUCCESS: Lock the node *)
  ELSE UNCHANGED vars (* CAS FAIL: Node is occupied *)
\end{tla}
\begin{tlatex}
...
\@x{\@s{8.2} \.{\THEN} ExecuteAcquire ( d ,\, n )}%
\@y{%
  CAS SUCCESS: Lock the node 
}%
\@xx{}%
\@x{\@s{8.2} \.{\ELSE} {\UNCHANGED} vars}%
\@y{%
...
Next, we can examine the step by step committing phase. It preserves the structural consistency invariant because accessing any modified node requires interacting with the operation descriptor, enforcing participation in the next commit step and maintaining consistency.
